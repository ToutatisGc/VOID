package cn.toutatis.xvoid.spring.security.security.access.auth
...
class LocalUserService : VoidAuthService {
    ...
    override fun preCheckAccount(requestAuthEntity: RequestAuthEntity): Boolean {
        ...
        val loginAccountOps = redisTemplate.boundValueOps(StringToolkit.concat(LOGIN_ACCOUNT_SESSION_KEY, requestAuthEntity.sessionId))
        ...
        if (loginConfig.beforeLoginCheckUsername){
            val sessionOps = redisTemplate.boundValueOps(StringToolkit.concat(LOGIN_PRE_CHECK_KEY,requestAuthEntity.sessionId))
        ...
        if (loginConfig.loginRetryLimitEnabled){
            val loginLockOps = redisTemplate.boundValueOps(StringToolkit.concat(LOGIN_ACCOUNT_LOCKED_KEY,account))
