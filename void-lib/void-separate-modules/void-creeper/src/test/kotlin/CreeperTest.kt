...
class CreeperTest {
    ...
    fun commonCreeper(){
        ...
        val elements = document.select("a[href*=20230803]")

        if (lookOnlyToday){
            val date = Date()
            val regexTime = Time.regexTime(Time.YMD_HORIZONTAL_FORMAT_REGEX,date)
            System.err.println(regexTime)
        }
...
//        System.err.println(elementsByTag)
    }

    @Test
    fun testHttps(){
        val document = Jsoup.connect("http://gov.sina.com.cn/").get()
        System.err.println(document.select("a[href*=2023-08-03]"))
    }
